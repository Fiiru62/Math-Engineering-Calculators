...
function handleGlobalEvent(event) {
	...
	// No manejar si el modo de selección está activo
...
async function handleEditSave(updatedText) {
	const newText = updatedText
		// Reemplazando caracteres que hacen fallar al parser de Babel
		.replace(/</g, "&lt;")
		.replace(/>/g, "&gt;")
		.replace(/{/g, "&#123;")
		.replace(/}/g, "&#125;");
...
function showDisabledTooltip(targetElement, isImage = false) {
	...
	// Asegura que el tooltip no quede fuera de la pantalla con un margen de 5px
...
function enableEditMode() {
	// No habilitar si el modo de selección está activo
	if (document.getElementById("root")?.getAttribute("data-selection-mode-enabled") === "true") {
