...
function checkTagNameEditable(openingElementNode, editableTagsList = EDITABLE_JSX_TAGS) {
	...
	// Comprobación 1: Nombre directo (p. ej., <p>, <Button>)
	if (nameNode.type === 'JSXIdentifier' && editableTagsList.includes(nameNode.name)) {
		return true;
	}

	// Comprobación 2: Nombre de propiedad de una expresión miembro (p. ej., <motion.h1>, comprobar si "h1" está en editableTagsList)
...
function validateImageSrc(openingNode) {
	if (!openingNode || !openingNode.name || ( openingNode.name.name !== 'img' && openingNode.name.property?.name !== 'img')) {
		return { isValid: true, reason: null }; // No es una imagen, omitir validación
	}
...
export default function inlineEditPlugin() {
	return {
		...
		transform(code, id) {
				...
				traverseBabel.default(babelAst, {
					enter(path) {
						if (path.isJSXOpeningElement()) {
							...
							const elementNode = path.parentPath.node; // El propio JSXElement
							...
							// Condición 1: ¿El tipo de etiqueta del elemento actual es editable?
							...
							// Condición 2: ¿El elemento tiene hijos dinámicos o editables?
							if (t.isJSXElement(elementNode) && elementNode.children) {
								// Comprobar si el elemento tiene el atributo {...props} - deshabilitar edición si lo tiene
							...
							// Condición 3: El padre no es editable si tiene hijos JSX no editables y que no son iconos.
							...
							// Condición 4: ¿Algún elemento JSX ancestro también es editable?
			...
			// Actualiza el código fuente según los cambios recibidos del cliente
			...
			server.middlewares.use('/api/apply-edit', async (req, res, next) => {
				...
				req.on('end', async () => {
					...
					try {
						...
						// Encontrar nodo objetivo (nota: apply-edit usa column+1)
						...
						if (isImageElement) {
								// Manejar actualización del atributo src de la imagen
