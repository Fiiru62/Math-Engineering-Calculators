const ALLOWED_PARENT_ORIGINS = [
	'http://localhost:4000',
	'https://app.netlify.com/projects/proyecto-final-md-jalau/overview'
];
...
function getFilePathFromNode(node) {
	...
	// Recorrer hacia arriba el árbol Fiber para encontrar metadatos de origen
...
function getPathToElement(element) {
	...
	let current = element;
	let depth = 0;
	const maxDepth = 20; // Evitar bucles infinitos
...
function handleMouseMove(event) {
	...
	// Solo actualizar si estamos pasando el cursor sobre un elemento diferente
	if (currentHoverElement !== element) {
		currentHoverElement = element;

		// Mostrar el contorno del elemento
		showOverlay(element);
	}
}
